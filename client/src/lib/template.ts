...
3. 出力
   ...
   ・**1行は短く（目安: 80文字程度）し、改行を多めにする（長い段落は禁止）**
   ・箇条書きは「1行=1論点」にする
...
5. VendorDoc が (なし) でない場合（契約書/仕様書監査）:
   ・VendorDoc の条項を監査観点（保存/学習利用/再委託/監査権/越境移転/削除/ログ/事故対応）でタグ付けし、各観点について「契約側の記載（引用）」「不足/曖昧な点（確認事項）」「対応するガイドライン要求（引用番号付き）」を対比して整理する
...
`;
  ...
  if (difficultyLevel === 'standard') {
    outputFormatSection += `
■ サマリー
結論: （まずここだけ読む。1行で、契約条項から言える結論を最短で書く）
前提: AI_in_scope に従い、AI/LLMが対象に含まれるかを最初に明示する（no の場合、医療AI固有の論点を勝手に適用しない）

【責任分界（契約書）】（VendorDoc がある場合は必ず先に出す）
・医療機関（甲）: （責任/負担/義務を1行で）
・ベンダー（乙/丙）: （責任/提供範囲を1行で）
・免責/賠償上限: （あれば1行で）

【ガイドライン上の確認（要点）】
・追加で確認すべき論点を2〜4点（1行ずつ）

【要確認（不足前提）】（判断不能(要確認)の場合のみ）
・足りない前提条件（例: 外部送信の有無、保存の有無、学習への利用、ログ保持、再委託先、事故時連絡）を3〜6点（1行ずつ）
...
export function generatePrompt(config: AppConfig, extSettings?: ExtendedSettings): string {
  ...
  // Replace AI_in_scope (guard against mixing up AI and non-AI contracts)
